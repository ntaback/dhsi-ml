...
review_words <- reviews %>% select(review_id, business_id, stars, text) %>% unnest_tokens(word,text) %>% filter(!word %in% stop_words$word, str_detect(word, "^[a-z']+$"))
...
Case Study #1 - Supervised Machine Learning: Predicting e-mail spam using logistic regression
...
- A classification tree predicts that each observation belongs to the most commonly occurring class of training observations in the region to which it belongs.

- The classification tree algorithm segments the features so that the "best" prediction is ultimately made.

```{r,fig.width=10,fig.height=11,eval=FALSE,echo=FALSE}
library(rpart)
library(tree)
library(maptree)
library(ROCR)
...
Heart <- read_csv("~/Dropbox/Docs/DHSI-2017/day2/Heart.csv")
Heart <- Heart[,2:15]

heart.tree <- rpart(AHD ~ ., method = "class", data = Heart)
draw.tree(heart.tree, cex = 0.8, nodeinfo = TRUE, col = gray(0:8/8))
```


# Case Study 2 - Supervised Machine Learning: Predicting e-mail spam using classification trees
...
draw.tree(model.rpart, cex = 0.8, nodeinfo = TRUE, col = gray(0:8/8))
