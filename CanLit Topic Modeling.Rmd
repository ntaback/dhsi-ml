...
ap_topics_15 <- tidy(ap_lda_30, matrix="beta")
ap_topics_15
...
CanLit_DTM <- DocumentTermMatrix(CanLit_corpus)
CanLit_DTM
```
...
The problem with this matrix is that it is absolutely huge. Note that the Sparsity of this DTM is 95%. That means approximately 95% of the cells in the matrix are empty (because so many of our words only appear in a handful of issues). The bigger the matrix, the more memory and time it will take to run our topic modeling algorithm so we need to figure out a way to shrink our matrix a bit. 

We can actually take two approaches to reducing the size of our matrix. We can use the TF-IDF to exclude terms that aren't very important to the matrix or we can use removeSparseTerms() to similarly shrink the size of the matrix. The first option is probably preferred given that it is less of a blunt approach than the second. Let's try both approaches:
...
```{r}
CanLit_DTM_Slim2 <- DocumentTermMatrix(CanLit_corpus, control = list(removePunctuation = TRUE, stopwods = TRUE ))
CanLit_DTM_Slim2 <- removeSparseTerms(CanLit_DTM, 0.8)
CanLit_DTM_Slim2
```

Quanteda also actually gives us tools for topic modeling:
...
CanLit_lda_5 <- LDA (Temp_DTM,k)
CanLit_lda_5_posterior <- posterior (CanLit_lda_5)
